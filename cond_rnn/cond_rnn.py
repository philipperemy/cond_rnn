...
class ConditionalRNN:
    ...
    def __call__(self, inputs, cond, *args, **kwargs):
        """
        ...
        if isinstance(cond, list):  # multiple conditions.
            ...
            init_state_list = []
            for cond in cond:
                init_state_list.append(tf.keras.layers.Dense(units=self.units)(cond))
            ...
            if cond is not None:
                self.init_state = tf.keras.layers.Dense(units=self.units)(cond)
