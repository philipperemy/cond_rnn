...
# can be sped up by resampling. ARIMAX is not really suited to work with patterns on a fine 
# [time scale](https://stackoverflow.com/questions/63438979/python-pmdarima-autoarima-does-not-work-with-large-data).
# It is mainly used here as benchmark. Calculations can be done on a laptop if
# seasonality is disabled. I used a [p3.2xlarge](https://aws.amazon.com/ec2/instance-types/p3/)
# from AWS but did not get better results with seasonality turned on.
# PMDArima can not exploit the GPUs, so using an instance with GPU does not add that much value.
